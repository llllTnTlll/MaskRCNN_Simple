...
import numpy as np
from bbox_ops import apply_box_deltas_graph, clip_boxes_graph, trim_zeros_graph, overlaps_graph, box_refinement_graph, norm_boxes_graph
...
class DetectionTargetLayer(tf.keras.layers.Layer):
    ...
    def call(self, proposals, gt_class_ids, gt_boxes, gt_masks):
        ...
        for i in range(self.batch_size):
            ...
            # if tf.greater(tf.shape(positive_overlaps)[1], 0):
            #     roi_gt_box_assignment = tf.argmax(positive_overlaps, axis=1)
            # else:
            #     roi_gt_box_assignment = tf.cast(tf.constant([]), tf.int64)
